...
void initAnswers(){
	//step1: dp数组的含义
	//dp[i][j]表示i颗蛋，尝试j次，在最坏的情况下，最多可以处理多高的建筑物
...
int nextTestFloor(int hFloor, int eggsLeft, int lastSurvival,int lastBreak){
    ...
    //已经确定临界楼层 
	if (lastBreak - lastSurvival <= 1) {
        return -1;
    }
    //找到第一个比“楼层”大的dp数组的值 
	int j = 1;
    while (j < 600 && dp[eggsLeft][j] < lastBreak - lastSurvival - 1) {
        j++;
    }
    if (j >= 600) {
        j = 599;
    ...
    int step = 0;
    if (eggsLeft > 1) {
        step = dp[eggsLeft-1][j-1];
    }
    return lastSurvival + step + 1;
}
