...
int weightedPathLength(BinTree<int>* bt){
    //带权路径 = sigma 叶子结点 x 深度
	int sum = 0;
	int depth = 0;
	//使用带标记的层序遍历，在遍历根节点时把特殊标记NULL入队，NULL出队代表一层遍历结束，此时再次入队。  
    ...
    while(!q.isEmpty()){
		...
		if(temp == NULL){
			if(q.isEmpty()){
				break;              //输出特殊标记的同时，如果队列是空的，说明没东西了，终止 
			}
			depth++;
			q.enQueue(NULL);		//遇到特殊标记说明遍历完了一层，NULL出队再入队，depth++ 
		}
        else{
        	if(temp->getLChild()==NULL && temp->getRChild()==NULL){
				sum += temp->getData() * depth;   //既没有左孩子也没有右孩子就是叶子，所以是叶子结点，并入带权路径 	
			}
