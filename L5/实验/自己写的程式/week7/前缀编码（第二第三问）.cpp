...
bool isPrefixCode(string* codes, int numCodes){
	...
	for(int i=0;i<numCodes-1;i++){
    	for(int j=i+1;j<numCodes;j++){
    		...
    		if(leni > lenj){
    			if(codes[j] == codes[i].substr(0,lenj)){
    				return 0;
				}
			}
			else if(leni == lenj){
				...
				if(codes[i] == codes[j].substr(0,leni)){
    				return 0; 
				}
			}
		}
...
void printDecode(char* charSet, string* codeSet, int numCodes, string encodedStr){
    //使用双循环，用idx表示encodedStr的遍历
	int idx = 0; 
	while(idx<encodedStr.length()){
		for(int i=0;i<numCodes;i++){
			if(encodedStr.substr(idx,codeSet[i].length()) == codeSet[i]){
				cout<<charSet[i];
				idx += codeSet[i].length();
			} 
		}
	} 
}
