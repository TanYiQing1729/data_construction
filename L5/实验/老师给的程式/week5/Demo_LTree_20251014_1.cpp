    ...
    public:
        ...
        LTree<T>* getChild(T x){
            for(int i=0;i<children.getLength();i++){
                LTree<T>* tmpTree = children.getEleAtPos(i);
                if(tmpTree->data == x)
                    return tmpTree;
            }
            return nullptr;
        ...
        void printPreOrder(){
            ...
            for(int i=0;i<children.getLength();i++){
                LTree<T>* tmpTree = children.getEleAtPos(i);
                tmpTree->printPreOrder();
            }
        }
        void printPostOrder(){
            for(int i=0;i<children.getLength();i++){
                LTree<T>* tmpTree = children.getEleAtPos(i);
                tmpTree->printPostOrder();
            }
            cout << data << " ";
        }
        void printLevelOrder(){
            LinkQueue<LTree<T>*> helper;
            helper.enQueue(this);
            while(!helper.isEmpty()){
                LTree<T>* tmpTree = helper.deQueue();
                cout << tmpTree->data << " ";
                for(int i=0;i<tmpTree->children.getLength();i++){
                    helper.enQueue(tmpTree->children.getEleAtPos(i));
                }
            }
        }
};
